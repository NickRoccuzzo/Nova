...
def update_graph(symbol, show_unusual):
    ...
    # 2) Build combined OI_OI array for marker sizing
    ...
    fig = go.Figure([
        ...
        go.Bar(
            x=idx, y=df['call_OI_sum'], name='Call OI Sum', yaxis='y2',
            opacity=0.6,
            hovertemplate='%{y}<extra></extra>'
        ),
        go.Bar(
            x=idx, y=df['put_OI_sum'], name='Put OI Sum', yaxis='y2',
            opacity=0.6,
            hovertemplate='%{y}<extra></extra>'
        )
    ])
    ...
    if 'yes' in show_unusual:
        ...
        if not uv.empty:
            ...
            for _, row in uv.iterrows():
                ...
                if key in pos_map:
                    # swap: calls below arrow (arrow up), puts above arrow (arrow down)
                    if row['side'].lower() == 'call':
                        ax, ay = 0, -30   # arrow points up, square below
                        border = 'green'
                    else:
                        ax, ay = 0, 30  # arrow points down, square above
                        border = 'red'
                    fig.add_annotation(
